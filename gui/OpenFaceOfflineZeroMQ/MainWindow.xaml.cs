...
///////////////////////////////////////////////////////////////////////////////
/*
This version output features to ZeroMQ
*/
using System;
...
namespace OpenFaceOffline
{
    ...
    public partial class MainWindow : Window
    {
        ...
        ZContext zero_mq_context;
        ZSocket zero_mq_socket;
        ...
        private volatile int frameNumber = 0;
        ...
        private void ProcessSequence(SequenceReader reader)
        {
            ...
            while (!gray_frame.IsEmpty)
            {
                ...
                detection_succeeding = landmark_detector.DetectLandmarksInVideo(frame, face_model_params, gray_frame);
        ...
        private void ProcessIndividualImages(ImageReader reader)
        {
            ...
            while (reader.isOpened())
            {
                ...
                {
                    detection_succeeding = landmark_detector.DetectFaceLandmarksInImage(frame, face_detections[i], face_model_params, gray_frame);
            ...
            bool new_image, bool multi_face, float fx, float fy, float cx, float cy, double progress)
        {
            ...
            visualizer.SetObservationGaze(gazeCams.Item1, gazeCams.Item2, landmark_detector.CalculateAllEyeLandmarks(), landmark_detector.CalculateAllEyeLandmarks3D(fx, fy, cx, cy), confidence);
